...
void MTRGInterstitialAdShowModal(uint32_t adId)
{
    [[NSOperationQueue mainQueue] addOperationWithBlock:^
    {
        mtrg_unity_tracer_d(@"MTRGInterstitialAdShowModal, adId = %d", adId);

        UIViewController *controller = mtrg_unity_topViewController();
        if (controller)
        {
            MTRGInterstitialAd *interstitialAd = mtrg_unity_findInterstitialAdWithAdId(adId, @"MTRGInterstitialAdShowModal");
            if (interstitialAd)
            {
                [interstitialAd showModalWithController:controller];
            }
        }
        else
        {
            mtrg_unity_tracer_e(@"MTRGInterstitialAdShowModal - UI not found");
        }
    }];
}
...
void MTRGInterstitialAdSetTrackingLocationEnabled(uint32_t adId, bool value)
{
    mtrg_unity_tracer_d(@"MTRGInterstitialAdSetTrackingLocationEnabled, adId = %d", adId);

    MTRGInterstitialAd *interstitialAd = mtrg_unity_findInterstitialAdWithAdId(adId, @"MTRGInterstitialAdSetTrackingLocationEnabled");
    if (interstitialAd)
    {
        [interstitialAd setTrackLocationEnabled:(BOOL) value];
    }
}
