...
uint32_t MTRGStandardAdCreate(uint32_t slotId, bool isRefreshAd, uint32_t adSize)
{
    ...
    MTRGAdView *adView = [[MTRGAdView alloc] initWithSlotId:slotId withRefreshAd:isRefreshAd adSize:adViewSize];
    if (adView)
    {
...
void MTRGStandardAdSetTrackingLocationEnabled(uint32_t adId, bool value)
{
    mtrg_unity_tracer_d(@"MTRGStandardAdSetTrackingLocationEnabled, adId = %d", adId);

    MTRGAdView *adView = mtrg_unity_findStandardAdWithAdId(adId, @"MTRGStandardAdSetTrackingLocationEnabled");
    if (adView)
    {
        [adView setTrackLocationEnabled:(BOOL) value];
    }
}